#ifndef SPFSS_SENDER_FP_BLAKE3_H__
#define SPFSS_SENDER_FP_BLAKE3_H__

// SPFSS Sender using BLAKE3 for GGM tree expansion (no AES-NI dependency)
...
template <typename IO> class SpfssSenderFpBlake3 {
public:
  block seed;
  block *ggm_tree, *m;
  __uint128_t delta;
  ...
  SpfssSenderFpBlake3(IO *io, int depth_in) {
  ...
  // Generate GGM tree using BLAKE3-based PRG
  void ggm_tree_gen(block *ot_msg_0, block *ot_msg_1, __uint128_t *ggm_tree_mem,
    ...
    TwoKeyPRP_Blake3 *prp = new TwoKeyPRP_Blake3(zero_block, makeBlock(0, 1));
...
#endif // SPFSS_SENDER_FP_BLAKE3_H__
